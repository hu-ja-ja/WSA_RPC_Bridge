...
| `mise run build` | `infisical run -- pnpm generate-licenses && infisical run -- pnpm tauri build` | デスクトップ版リリースビルド |
| `mise run lint` | `pnpm lint` (oxlint) | フロントエンドの静的解析 |
| `mise run test` | `cargo test` (src-tauri 内) | Rust ユニットテスト |
| `mise run android-test` | `./gradlew test` / `gradlew.bat test` (src-tauri/gen/android 内、OS により切替) | Android ユニットテスト (Robolectric) |
...
| `mise run docs-dev` | `pnpm dev` (site 内) | ドキュメントサイトの開発サーバー |
| `mise run docs-build` | `pnpm build` (site 内) | ドキュメントサイトのビルド |
