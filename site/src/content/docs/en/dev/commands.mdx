...
|---|---|---|---|
| Start development | `mise run dev` | `mise tauri android dev` | [Develop](#develop) |
| Release build | `mise run build` | `mise tauri android build --apk -t aarch64` | [Build](#build) |
| Lint / test | `mise run lint` / `test` / `android-test` (shared) | — | [Verify](#verify) |
| Misc (licenses / docs / preview / generic tauri) | `mise run generate-licenses` / `docs-dev` / `docs-build` / `pnpm preview` (shared) | `mise tauri info` etc. | [Misc](#misc) |

Argument passing, no-login alternatives, and CI mapping are grouped under [Notes](#notes).
