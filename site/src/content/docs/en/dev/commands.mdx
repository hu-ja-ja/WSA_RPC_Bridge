...
- **On a fresh checkout, `mise run pipeline:codegen` is required first** (the generated Kotlin code, gradle config, and `libs/rustls-platform-verifier.aar` under `gen/android` are not tracked in git. `gen/android` itself is tracked, so `android init` is not needed).
