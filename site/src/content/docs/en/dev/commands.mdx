...
| `mise run build` | `infisical run -- pnpm generate-licenses && infisical run -- pnpm tauri build` | Desktop release build |
| `mise run lint` | `pnpm lint` (oxlint) | Frontend linting |
| `mise run test` | `cargo test` (in src-tauri) | Rust unit tests |
| `mise run android-test` | `./gradlew test` / `gradlew.bat test` (in src-tauri/gen/android, depending on OS) | Android unit tests (Robolectric) |
