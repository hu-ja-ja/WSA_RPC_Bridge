...
- **On a fresh checkout, `mise run pipeline:codegen` is required first** (the generated Kotlin code, gradle config, and `libs/rustls-platform-verifier.aar` under `gen/android` are not tracked in git. `gen/android` itself is tracked, so `android init` is not needed. See [Pipeline](../actions/pipeline/)).
...
Build commands used by releases ([release.yml](https://github.com/hu-ja-ja/WSA_RPC_Bridge/blob/main/.github/workflows/release.yml) / [Release](../actions/)):
