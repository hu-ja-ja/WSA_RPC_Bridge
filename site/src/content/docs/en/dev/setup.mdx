...
- `gen/android` itself is tracked in git, so `android init` is not needed. `pipeline:codegen` fills in what a fresh checkout lacks (generated Kotlin under `generated/`, `tauri.*.gradle*`, `libs/rustls-platform-verifier.aar`; see [Pipeline](../actions/pipeline/)).
- Dev builds work without `discord_partner_sdk.aar`, which cannot be committed for licensing reasons (optional dependency, invoked via reflection). See the [release](../actions/) for release-time retrieval.
