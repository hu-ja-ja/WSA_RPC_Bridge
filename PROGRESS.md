...
- [x] ポーリングループで自動更新
- [x] RPC オン/オフ トグル (Kobalte Switch)
- [x] localStorage による設定永続化（再起動後も維持）
- [x] タブ切り替え式レイアウト (Kobalte Tabs)
- [x] エラーハンドリング改善（グレースフルデグラデーション、エラートースト）
- [x] UIデザイン刷新（ミニマル・クリーン、テーマ対応）
